...
func updateRemoteMysql(ip string, winIP string) error {
	host := fmt.Sprintf("--host=%s", ip)
	user := "--user=dbadmin"
	password := "-pDatamotive@123"
	database := "wordpress"
	command := fmt.Sprintf(`-e UPDATE wp_options SET option_value='http://%s/wordpress' WHERE option_value='http://51.89.240.69/wordpress'`, winIP)
	cmd := exec.Command("mysql", host, user, password, database, command)
	out, err := cmd.CombinedOutput()
	log.Println("mysql run output: ", string(out))
	if err != nil {
		log.Println("Error while running script: ", err)
		return err
	}
	log.Println("mysql run output: ", out)
	return nil
}
...
func runWindowsCommand(ip string, username string, password string, ubuntuIP string) error {
	...
	command := `get-content C:\\xampp\\htdocs\\wordpress\\dbconfig.php | %{$_ -replace "51.89.240.70", "` + ubuntuIP + `"}
	sc stop Apache2.4
	sc start Apache2.4`
...
func main() {
	...
	for _, machine := range *machineList {
		if strings.Contains(machine.Name, "Ubuntu") {
			winMachineIP := getVMIP(*machineList, "win-2012-website")
			err = updateRemoteMysql(machine.IP, winMachineIP)
			if err != nil {
				return
			}
		} else if strings.Contains(machine.Name, "win") {
			ubunutIP := getVMIP(*machineList, "prod-ubuntu-16-04")
