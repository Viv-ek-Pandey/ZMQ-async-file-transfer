...
# Execution Level: VM
# =================================
# This script is invoked after VM recovery is completed.
#
...
main() {
    ...
    local target_id=$(echo "$INPUT_JSON" | jq -r '.targetID' | tr -d '`' | sed 's/null//g' | tr -d '\n')
    ...
    log "Recovered VM ID: $target_id Name: $source_name IP: $target_ip"
