...
# ==================================
# This script is invoked after the replication of all workloads has started.
#
...
main() {
    ...
    log "Post-Plan Replication Script started at: $(date '+%Y-%m-%d %H:%M:%S')"
    ...
    # Extract the source ID of the first VM in the list
    local source_id=$(echo "$INPUT_JSON" | jq -r '.vms[0].sourceID' | tr -d '`' | sed 's/null//g' | tr -d '\n')

    # Log the extracted source ID
